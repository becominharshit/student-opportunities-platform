...
export function DiscoveryShell({ children, authenticated=false, unreadCount=0 }: { children: ReactNode; authenticated?:boolean; unreadCount?: number }) {
  return <><header className="border-b border-border"><nav aria-label="Main" className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-5 py-5 sm:px-8"><Link href="/" className="font-semibold">Student Opportunities</Link><div className="flex flex-wrap items-center gap-6"><Link href="/explore" className="underline underline-offset-4">Explore</Link>{authenticated?<><Link href="/for-you" className="underline underline-offset-4">For You</Link><Link href="/assistant" className="underline underline-offset-4">Assistant</Link><Link href="/saved" className="underline underline-offset-4">Saved</Link><Link href="/notifications" className="inline-flex items-center gap-1.5 underline underline-offset-4">Notifications{unreadCount > 0 && <span aria-label={`${unreadCount} unread notifications`} className="inline-flex items-center justify-center rounded-full bg-primary px-1.5 py-0.5 text-xs font-semibold text-primary-foreground leading-none">{unreadCount > 99 ? "99+" : unreadCount}</span>}</Link><Link href="/account" className="underline underline-offset-4">Account</Link></>:<Link href="/login" className="underline underline-offset-4">Sign in</Link>}</div></nav></header><main id="main-content" className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-16">{children}</main><footer className="mx-auto max-w-6xl border-t border-border px-5 py-8 text-sm text-muted-foreground sm:px-8">Registration takes place on the organizer’s website. Check the official rules before applying. · <Link href="/submit-event" className="underline underline-offset-4">Submit an opportunity</Link></footer></>;
}
...
export function EventDetailContent({ event, personalization, saveState }: { event: PublicEvent; personalization?: ReactNode; saveState?:SaveState }) {
  ...
  return <DiscoveryShell><Link href="/explore" className="underline underline-offset-4">← Explore all events</Link>
    ...
    {personalization}
    <div className="mt-10 grid items-start gap-10 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]"><aside aria-label="Official registration" className="min-w-0 border-t-2 border-primary bg-muted p-6 lg:order-last"><h2 className="text-xl font-semibold">Visit the organizer</h2><p className="mt-4 text-sm font-semibold">Registration deadline</p><p className="mt-1"><Deadline event={event} /></p><p className="mt-4 text-sm">Registration: {label(event.registration_status, "status unknown")}</p><div className="mt-5 flex flex-col gap-3"><ExternalLink url={event.registration_url} primary>{registrationOpen ? "Register on official website" : "View official registration page"}</ExternalLink><ExternalLink url={event.official_url}>Official event website</ExternalLink></div><EventCalendarSection event={event} /><p className="mt-4 text-sm text-muted-foreground">These links leave Student Opportunities and open in a new tab. Registration is handled by the organizer.</p></aside><div className="min-w-0 space-y-10">
