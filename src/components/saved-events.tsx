...
export function SavedContent({value,after}:{value:SavedResult;after?:string}){
 ...
 {value.items.length?<ul className="mt-6 grid gap-8 md:grid-cols-2">{value.items.map(event=><li className="min-w-0" key={event.id}><EventCard event={event} saveState={{kind:'ready',ids:value.items.map(e=>e.id)}} returnTo={'/saved'+(after?'?after='+encodeURIComponent(after):'')} showCalendarActions={true}/></li>)}</ul>:<section className="my-10"><h2 className="text-2xl font-semibold">{value.hasAny?'No currently available saved opportunities.':"You haven't saved any opportunities yet."}</h2></section>}
