...
Grounded AI Assistant adds an authenticated conversational assistant (/assistant) and API route (/api/assistant) strictly grounded in canonical database records with structured authoritative platform fact cards, link disarming, prose contradiction detection, and durable PostgreSQL-backed rate limits and quotas in private.assistant_usage (10 req/60s fixed window, 50 req/UTC day, 1 in-flight request per user lease locking).
No automated external source integration is implemented (C09–C11, C17 deferred).
...
Automatic external discovery/sync is **deferred, not completed**; C07 research and C08/C08.5 infrastructure remain preserved. C12–C16, C18, Calendar Export, Notifications, Organizer Submissions, and Grounded AI Assistant are complete.

See [Grounded AI Assistant review and validation](docs/section-36-ai-assistant-review.md). Exactly one additive migration `20260924000300_ai_assistant_limits.sql` applied. Run `npm run test:assistant`, `npm run test:assistant:performance`, `npm run test:assistant:ui` (Playwright across 320/390/768/1280px), and `npm run test:assistant:hosted` for isolated and hosted checks.
